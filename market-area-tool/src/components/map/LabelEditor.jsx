...
const LabelEditor = ({
    ...
    const collectAllLabels = () => {
      ...
      const collectFromLayer = (layer) => {
        ...
        layer.graphics.items.forEach(graphic => {
          ...
          if (!processedIds.has(labelId) &&
              (graphic.attributes?.isLabel === true || (graphic.symbol && graphic.symbol.type === "text")))
          {
            labels.push(graphic);
      ...
      console.log(`[LabelEditor] Collected ${labels.length} unique labels for editing`);
      setAllLabels(labels);
      return labels;
    };
        ...
        clickHandlerRef.current = mapView.on('click', (event) => {
          ...
          mapView.hitTest(screenPoint).then(response => {
            const labelHits = response.results?.filter(result =>
              result.graphic?.attributes?.isLabel === true || (result.graphic?.symbol && result.graphic?.symbol.type === "text")
            );
            ...
            if (labelHits && labelHits.length > 0) {
              ...
              if (selectedLabel !== hitLabel) {
                  console.log('[LabelEditor] Selected label:', getLabelId(hitLabel), hitLabel);
                  setSelectedLabel(hitLabel);
                  setFontSize(hitLabel.symbol?.font?.size || 10);
                  setLabelText(hitLabel.symbol?.text || '');
                  setPosition({ x: hitLabel.symbol?.xoffset || 0, y: hitLabel.symbol?.yoffset || 0 });
                  setSelectAllMode(false);
                  setIsTextEditing(false); // Turn off text editing if selecting new label
              } else {
                   console.log('[LabelEditor] Re-clicked the same label.');
                  // Optional: Could toggle text edit on re-click?
                  // setIsTextEditing(true);
              }

            } else {
              if (selectedLabel) { // Only deselect if one was selected
                console.log('[LabelEditor] Clicked outside, deselecting label.');
  ...
  useEffect(() => {
    ...
    const getLabelId = (label) => {
        ...
        if (label.attributes?.labelId) return `${layerPrefix}explicit-${label.attributes.labelId}`;
        ...
        if (label.attributes?.uid) return `${layerPrefix}uid-${label.attributes.uid}`;
        ...
        if (label.geometry) {
        const textPart = label.symbol?.text?.substring(0, 10).replace(/\s+/g, '_') || 'no_text';
        return `${layerPrefix}geom-${label.geometry.x?.toFixed(2)}-${label.geometry.y?.toFixed(2)}-${textPart}`;
        }
        
        // Last resort
        return `${layerPrefix}unknown-${Math.random().toString(36).substring(2, 9)}`;
    };
    ...
    const triggerLabelRefresh = (labelIds = []) => {
        ...
        console.log('[LabelEditor] Triggering refresh via labelManager for IDs:', labelIds);
        ...
        setTimeout(() => {
            ...
            if (!result.success) {
                console.warn('[LabelEditor] Label manager refresh failed:', result.message);
                
                // Try direct refresh as fallback
                if (selectedLabel && selectedLabel.layer && typeof selectedLabel.layer.refresh === 'function') {
                console.log('[LabelEditor] Falling back to direct layer refresh for selected label');
                selectedLabel.layer.refresh();
                }
                else if (activeLayer && typeof activeLayer.refresh === 'function') {
            ...
            if (activeLayer && typeof activeLayer.refresh === 'function') {
                console.log('[LabelEditor] Using layer refresh fallback after error');
                activeLayer.refresh();
            }
            }
        }, 0);
        ...
        console.warn('[LabelEditor] labelManager.refreshLabels not available. Using layer/view refresh fallback.');
        ...
        const labelsToProcess = allLabels.filter(lbl => labelIds.includes(getLabelId(lbl)));
        ...
        layersToRefresh.forEach(layer => {
            ...
            console.log(`[LabelEditor] Refreshing layer: ${layer.id || layer.title}`);
            layer.refresh();
            }
        });
        } else if (mapView?.graphics && typeof mapView.graphics.refresh === 'function') {
    ...
    const hideAllDuplicatesFor = (label) => {
        ...
        if (!labelId) {
        console.warn(`[LabelEditor] Cannot hide duplicates - unable to generate ID for label`);
        return;
        }
        ...
        if (!parentId) {
        console.warn(`[LabelEditor] Cannot hide duplicates - no parent ID for label ${labelId}`);
        return;
        }
        
        // First approach: Use labelManager if available
        if (labelManager && typeof labelManager._hideAllDuplicatesFor === 'function') {
        console.log(`[LabelEditor] Using labelManager to hide duplicates for label ${labelId}`);
        ...
        console.log(`[LabelEditor] Hid ${duplicatesFound} duplicate labels for parent ID ${parentId}`);
        }
    };
    ...
    const updateLabelProperties = (label, properties) => {
        ...
        if (properties.fontSize !== undefined && newSymbol.font?.size !== properties.fontSize) {
        ...
        if (properties.position !== undefined && 
            (newSymbol.xoffset !== properties.position.x || newSymbol.yoffset !== properties.position.y)) {
            newSymbol.xoffset = properties.position.x;
            newSymbol.yoffset = properties.position.y;
            changed = true;
            changes.push(`position: (${properties.position.x}, ${properties.position.y})`);
        }
        ...
        if (changed) {
            ...
            [labelId]: {
                ...
                fontSize: newSymbol.font?.size ?? (prev[labelId]?.fontSize ?? fontSize),
                position: {
            ...
            console.log(`[LabelEditor] Updated properties for label ${labelId}`, changes.length > 0 ? `(${changes.join(', ')})` : '');
        } else {
            console.log(`[LabelEditor] No effective change detected for label ${labelId}`);
        }
        } catch (error) {
        console.error(`[LabelEditor] Error updating properties for label ${labelId}:`, error);
        }
    };
  ...
  const updateFontSize = (newSize, targetLabel = null) => {
    ...
    if (selectAllMode) {
      ...
      allLabels.forEach(label => {
        updateLabelProperties(label, { fontSize: newSize });
        updatedIds.push(getLabelId(label));
      });
      // Trigger a single refresh for all potentially updated labels after the loop
      // Note: updateLabelProperties already calls triggerLabelRefresh individually.
      // A bulk refresh might be slightly more efficient if the manager supports it well.
      // triggerLabelRefresh(updatedIds); // Optional: Bulk refresh instead of individual
      console.log(`[LabelEditor] Updated font size to ${newSize} for all ${allLabels.length} labels`);
    } else {
      const labelToUpdate = targetLabel || selectedLabel;
      if (labelToUpdate) {
        updateLabelProperties(labelToUpdate, { fontSize: newSize });
        // Refresh is handled within updateLabelProperties
  ...
  const updateLabelText = (newText, targetLabel = null) => {
    ...
    if (labelToUpdate) {
      ...
      updateLabelProperties(labelToUpdate, { text: newText });
      // Refresh is handled within updateLabelProperties
      console.log(`[LabelEditor] Updated text for selected label`);
    }
  };

  // --- Enable label moving mode (with Y-axis fix) ---
  const enableMovingMode = () => {
    ...
    moveDownHandlerRef.current = mapView.on('pointer-down', (event) => {
      if (!selectedLabel) { // Safety check
        console.warn("[LabelEditor] Pointer Down ignored: selectedLabel became null.");
        setIsMoving(false);
        if (mapView.container) mapView.container.style.cursor = 'pointer';
        if (moveMoveHandlerRef.current) moveMoveHandlerRef.current.remove();
        if (moveUpHandlerRef.current) moveUpHandlerRef.current.remove();
      ...
      labelStartOffsetRef.current = {
       ...
       console.log('[LabelEditor] Drag Start - Screen:', dragStartRef.current, 'Initial Offset:', labelStartOffsetRef.current);
    });

    // --- Pointer Move ---
    moveMoveHandlerRef.current = mapView.on('pointer-move', (event) => {
      if (!dragStartRef.current || !labelStartOffsetRef.current || !selectedLabel) return;
      ...
      // *** FIX: Invert dy for correct vertical movement ***
      const newOffset = {
        x: labelStartOffsetRef.current.x + dx,
        y: labelStartOffsetRef.current.y - dy // Invert vertical delta
      };

      // Update UI state *before* updating properties to ensure smooth feedback
      setPosition(newOffset);
      // Update graphic symbol and trigger refresh via centralized function
      updateLabelProperties(selectedLabel, { position: newOffset });

       // Optional: Throttled logging
       // const now = Date.now();
       // if (!lastMoveLogTime || now - lastMoveLogTime > 50) {
       //     console.log(`[LabelEditor] Moving - Delta: (${dx.toFixed(0)}, ${dy.toFixed(0)}), New Offset: (${newOffset.x.toFixed(0)}, ${newOffset.y.toFixed(0)})`);
       //     lastMoveLogTime = now;
       // }

    });
    // let lastMoveLogTime = 0; // Optional: For throttling move logs
    ...
    moveUpHandlerRef.current = mapView.on('pointer-up', (event) => {
      ...
      console.log('[LabelEditor] Finished moving label. Final Offset State:', position);
      // Final refresh trigger might be redundant if updateLabelProperties worked, but can be added as safety
      // if (selectedLabel) triggerLabelRefresh([getLabelId(selectedLabel)]);
    });
  };
  ...
  const toggleSelectAllMode = () => {
    ...
    /**
     * Finalizes label edits and saves them to storage
     * @param {boolean} silent Whether to show alerts (false) or run silently (true)
    ...
    const finalizeLabels = (silent = false) => {
        ...
        console.log(`[LabelEditor] Finalizing ${Object.keys(editedLabels).length} tracked label edits.`);
    
        try {
        // Complete any active text editing
        if (isTextEditing && selectedLabel) {
            updateLabelProperties(selectedLabel, { text: labelText });
            setIsTextEditing(false);
        }
        ...
        if (labelManager && typeof labelManager.savePositions === 'function') {
            try {
            const result = labelManager.savePositions(editedLabels, true); // Force localStorage usage
        ...
        if (!saveSuccessful) {
            // Fallback: Use local storage directly
            console.warn("[LabelEditor] Label manager save failed or unavailable. Using localStorage fallback.");
            const existingPositions = JSON.parse(localStorage.getItem('customLabelPositions') || '{}');
            ...
            positionsToSave[id] = {
                ...
                text: data.text,
                visible: data.graphic?.visible ?? true
            };
            });
            const mergedPositions = { ...existingPositions, ...positionsToSave };
            localStorage.setItem('customLabelPositions', JSON.stringify(mergedPositions));
            console.log('[LabelEditor] Saved/Merged label positions to localStorage');
        }
    
        // Mark all edited labels with permanent flag for the monitoring system
        Object.entries(editedLabels).forEach(([labelId, data]) => {
            if (data.graphic && data.graphic.attributes) {
            data.graphic.attributes._permanentEdit = true; // Add flag for permanent edits
            data.graphic.attributes._isEdited = true;
            
            // Remove any duplicate flags
            delete data.graphic.attributes._isDuplicate;
            delete data.graphic.attributes._duplicateOf;
            }
        });
    
        setEditedLabels({}); // Clear tracked edits
        setSelectedLabel(null);
        setLabelText('');
        setSelectAllMode(false);
        ...
        // Optionally trigger a final global refresh after saving
        triggerLabelRefresh();
  ...
  // --- Reset all changes ---
  const resetChanges = () => {
     if (!labelManager && Object.keys(editedLabels).length === 0) {
         console.log("[LabelEditor] No changes to reset.");
         return;
     }
     console.log('[LabelEditor] Resetting all label changes.');

     try {
        // Reset via manager first
        if (labelManager && typeof labelManager.resetAllLabelPositions === 'function') {
            labelManager.resetAllLabelPositions();
            console.log('[LabelEditor] Reset positions using label manager.');
        } else {
            console.warn('[LabelEditor] labelManager reset function not available. Clearing local storage and attempting manual reset.');
             // Clear storage fallback
            localStorage.removeItem('customLabelPositions');
             // Attempt manual reset by reapplying original labeling (tricky without manager)
             // For now, just trigger a full refresh
             if (activeLayer && typeof activeLayer.refresh === 'function') activeLayer.refresh();
        ...
        // Trigger a visual refresh after resetting state
        triggerLabelRefresh();
  ...
  const resetSingleLabel = () => {
    ...
    if (labelManager && typeof labelManager.resetLabelPosition === 'function') {
        try {
            ...
            console.error(`[LabelEditor] Error resetting label ${labelId} via manager:`, err);
        }
    }
    ...
    if (!resetSuccessful) {
        ...
        if (existingPositions[labelId]) {
            ...
            localStorage.setItem('customLabelPositions', JSON.stringify(existingPositions));
            console.log(`[LabelEditor] Reset label ${labelId} via localStorage removal.`);
    ...
    if (resetSuccessful) {
       ...
       setTimeout(() => triggerLabelRefresh([labelId]), 0);
        ...
        console.warn(`[LabelEditor] Could not confirm reset method for ${labelId}, visual update might be needed manually.`);
    }
    ...
    <div className="flex flex-col h-full bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100">
      ...
      <div className="flex-none p-4 border-b border-gray-200 dark:border-gray-700">
        ...
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
          Click label to edit, or use "Select All". Dragging map is disabled while moving labels.
        </p>
      </div>
      ...
      <div className="flex-1 p-4 overflow-y-auto space-y-6">
        ...
        {selectAllMode ? (
             ...
             <div>
                ...
                {isTextEditing ? (
                    <div className="flex items-center space-x-2">
                      <input
                        ref={textInputRef} type="text" value={labelText}
                        onChange={(e) => setLabelText(e.target.value)}
                        onKeyDown={(e) => {
                          if (e.key === 'Enter') { updateLabelText(e.target.value); setIsTextEditing(false); }
                    ...
                    <div className="flex justify-between items-center p-2 bg-gray-50 dark:bg-gray-800 rounded-md border border-gray-200 dark:border-gray-700 min-h-[38px]">
                      <p className="text-sm truncate flex-1 mr-2" title={labelText || selectedLabel.symbol?.text || ''}>
                        {labelText || selectedLabel.symbol?.text || '(No text)'}
                      </p>
                      <button onClick={() => setIsTextEditing(true)} className="p-1 rounded-full text-blue-600 hover:bg-blue-100 dark:text-blue-400 dark:hover:bg-blue-900/50" title="Edit Text"><Edit className="h-4 w-4" /></button>
                    </div>
                )}
            </div>
          ...
          <div className="flex flex-col items-center justify-center h-full text-center text-gray-500 dark:text-gray-400">
            ...
            <p className="mt-2 text-sm">Click a label on the map or use "Select All".</p>
          </div>
        )}
      </div>
